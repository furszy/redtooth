...
package iop_sdk.forum.discourge.com.wareninja.opensource.discourse;
...
import iop_sdk.forum.discourge.com.wareninja.opensource.discourse.utils.MyWebClient;
...
this.post('posts', { 'title': title, 'raw': raw, 'category': category, 'archetype': 'regular' }, function(error, body, httpCode) {
    callback(error, body, httpCode);
		...
		if (responseModel.meta.code==200) {
			//JSONObject jsonObject = new JSONObject(responseStr);
			responseModel.data = responseStr; //responseStr;
		}else {
			responseModel.data = responseStr;
		}
...
//		}
	...
	public ResponseModel replyToTopic(Map<String, String> parameters) {
		// SYNCHRONOUS function
		final String TAG = "replyToTopic";
		if (!parameters.containsKey("topic_id")) throw new IllegalArgumentException("Request parameters does not contains id (topic id)");
		if (!parameters.containsKey("raw")) throw new IllegalArgumentException("Request parameters does not contains raw (post data)");
		...
		//parameters.put("filter", FILTER.NEW_TOPIC.name());
		String responseStr = webClient.post(methodName, parameters,"application/x-www-form-urlencoded");
		ResponseModel responseModel = new ResponseModel();
		responseModel.meta.code = webClient.getHttpResponseCode();
		responseModel.data = responseStr;
		return responseModel;
	...
	public ResponseModel updateTopic(Map<String, String> parameters) {
		...
		String methodName = "";
		methodName += "/t/"+parameters.get("id");
		methodName = webClient.enrichMethodName(methodName, this.api_key, this.api_username);// append api_key and api_username
	...
	public ResponseModel updatePost(Map<String, String> parameters) {
		// SYNCHRONOUS function
		final String TAG = "updatePost";
		if (!parameters.containsKey("id")) throw new IllegalArgumentException("parameters not contains id");
		if (!parameters.containsKey("raw")) throw new IllegalArgumentException("parameters not contains raw");
		...
		String methodName = "";
		//methodName += "/t/"+parameters.get("id");
		methodName += "/posts/"+parameters.get("id")+".json";
		methodName = webClient.enrichMethodName(methodName, this.api_key, this.api_username);// append api_key and api_username

		String put = "post[raw=\""+parameters.get("raw")+"\"]&id="+parameters.get("id")+"&api_key="+api_key+"&api_username="+api_username;

		String requestUrl;
		if (methodName.startsWith(webClient.getWebServiceUrl())) {
			requestUrl = methodName;
		} else {
			requestUrl = webClient.getWebServiceUrl() + methodName;
		}

		System.out.println("PUT requestUrl : " + requestUrl);

		String responseStr = webClient.put(methodName,put,"application/x-www-form-urlencoded");
